...
function populateChain(chain) {
	...
	document.querySelectorAll('[data-field="strike"]').forEach(select => {
		...
		select.innerHTML = `<option value="">Select ${type === "CE" ? "call" : "put"} strike</option>` + quotes.map(quote => `<option value="${quote.strike}">${fmt(quote.strike, 0)} ${type} — ₹${fmt(quote.ltp)}</option>`).join("");
	});
}
...
loadExpiries(); refresh(); setInterval(refresh, 3000);
